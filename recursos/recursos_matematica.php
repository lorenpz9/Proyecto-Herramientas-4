...
<div class="container mt-5">
  <h2 class="text-danger">Recursos de Matemática</h2>
      ...
      <?php foreach ($recursos as $recurso): ?>
        <div class="col-md-4 mb-4">
          <div class="card h-100 border-0 shadow-sm" style="background-color:#F5F5DC;">
            <div class="card-body">
              <h5 class="card-title text-dark"><?= htmlspecialchars($recurso['titulo']) ?> <small class="text-muted">(<?= $recurso['tipo'] ?>)</small></h5>
              ...
              <?php if ($recurso['tipo'] === 'enlace'): ?>
                <a href="<?= $recurso['enlace'] ?>" target="_blank" class="btn btn-sm btn-primary">Ver Enlace</a>
              <?php else: ?>
                <a href="../<?= $recurso['archivo'] ?>" download class="btn btn-sm btn-success">Descargar</a>
              <?php endif; ?>
            </div>
          </div>
        </div>
